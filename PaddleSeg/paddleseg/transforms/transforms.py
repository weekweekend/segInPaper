...
class RandomAttack:
    ...
    def __init__(self, prob=0.5, max_sigma=0.1):
        self.prob = prob
    ...
    def __call__(self, im, label=None):
        ...
        if random.random() < self.prob:
             ...
             if random.random()<0.25:
                # 三次函数使得小值取的可能性更大
                var = (random.random()*0.4)**3
                im =  random_noise(im, mode='gaussian', var=var) * 255
                
             # 椒盐
             elif random.random()<0.5:
                amount = (random.random()*0.4+0.1)**3
                im = random_noise(im, mode='s&p',amount = amount ) * 255
             # pink 噪声
             elif random.random()<0.75:
                var = (random.random()*0.4+0.1)**3
                shape = im.shape
                nums = shape[0]*shape[1]*shape[2]
                y = var * powerlaw_psd_gaussian(1, nums).reshape(shape)
                im =  (im/255+y)*255
             # jpeg 压缩
             else:
                q = int(random.random()*50)
                params = [cv2.IMWRITE_JPEG_QUALITY, q]
                msg = cv2.imencode(".jpg", im, params)[1]
                msg = (np.array(msg)).tobytes() 
                im = cv2.imdecode(np.frombuffer(msg, np.uint8), cv2.IMREAD_COLOR)
        # file_path = 'atest/im{:d}.jpg'.format(int(time.time()))
